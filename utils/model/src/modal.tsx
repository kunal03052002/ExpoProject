...
import ErrorCircleIcon from "@mui/icons-material/ErrorOutlineRounded";
import InfoCircleIcon from "@mui/icons-material/InfoOutlined";
import SuccessCircleIcon from "@mui/icons-material/DoneAllOutlined";
import WarningCircleIcon from "@mui/icons-material/WarningOutlined";
import { AnimatePresence, motion } from "framer-motion";
import { forwardRef, ReactNode, useContext, useMemo } from "react";
import FocusLock from "react-focus-lock";
import { useHotkeys } from "react-hotkeys-hook";
import { Button } from "@mui/material";
import { TriggerProvider } from "@/utils/trigger";
  ...
  const normalIcon: ReactNode = useMemo(() => {
    if (type && title) {
      switch (type) {
        case "info":
          return (
            <InfoCircleIcon sx={{ color: globalColor("blue-03"), mr: "8px" }} />
          );
        case "error":
          return (
            <ErrorCircleIcon sx={{ color: globalColor("red-03"), mr: "8px" }} />
          );
        case "success":
          return (
            <SuccessCircleIcon
              sx={{ color: globalColor("green-03"), mr: "8px" }}
            />
          );
        case "warning":
          return (
            <WarningCircleIcon
              sx={{ color: globalColor("red-03"), mr: "8px" }}
            />
          );
        default:
          return <></>;
