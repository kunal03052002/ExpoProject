...
import InfoCircleIcon from "@mui/icons-material/Info";
import SuccessCircleIcon from "@mui/icons-material/CheckCircleOutline";
import WarningCircleIcon from "@mui/icons-material/Warning";
import { globalColor } from "../colorBuilder";
import { applyBoxStyle } from "../DropDown/style";
    ...
    const normalIcon: ReactNode = useMemo(() => {
      if (showIcon) {
        if (icon) {
          ...
          switch (type) {
            case "info":
              return <InfoCircleIcon sx={{ color: globalColor("blue-03") }} />;
            case "error":
              return <ErrorCircleIcon sx={{ color: globalColor("red-03") }} />;
            case "success":
              return (
                <SuccessCircleIcon sx={{ color: globalColor("green-03") }} />
              );
            case "warning":
              return (
                <WarningCircleIcon sx={{ color: globalColor("orange-03") }} />
              );
            case "normal":
            default:
              return <></>;
