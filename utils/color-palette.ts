...
import { globalColor, hasGlobalColor } from "./colorBuilder"
...
export const getColor = (color: string, step: string) => {
  ...
  if (!hasGlobalColor(`${color}-${step}`)) {
    const formatStep = transRule[step]
    const formatNum = formatStep ? formatStep : 6
    try {
    ...
    colorStyle = globalColor(`${color}-${step}`)
  }
  return colorStyle
}
