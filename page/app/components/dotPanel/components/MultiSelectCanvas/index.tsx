...
import { ILLA_MIXPANEL_EVENT_TYPE, trackInEditor } from "@/utils/mixpanelHelper"
...
export const MultiSelectCanvas: FC<MultiSelectCanvasProps> = (props) => {
  ...
  const dragConditionHandler= (e: OnDragStart<any>) => {
  ...
  const onDragStartHandler = (e: SelectoEvents["dragStart"]) => {
    ...
    currentCanvasStyle?.setProperty("--ImpaktApps-select-area-left", `${startX}px`)
    currentCanvasStyle?.setProperty("--ImpaktApps-select-area-top", `${startY}px`)
  ...
  const onDraggingHandler = (e: SelectoEvents["drag"]) => {
    ...
    currentCanvasStyle?.setProperty(
      "--ImpaktApps-select-area-left",
      `${currentXOrigin}px`,
    )
    currentCanvasStyle?.setProperty(
      "--ImpaktApps-select-area-top",
      `${currentYOrigin}px`,
    )
    currentCanvasStyle?.setProperty(
      "--ImpaktApps-select-area-width",
      `${rect.width}px`,
    )
    currentCanvasStyle?.setProperty(
      "--ImpaktApps-select-area-height",
      `${currentHeight}px`,
    )
  ...
  return (
    <Selecto
      ...
      dragCondition={(e:any)=>dragConditionHandler(e)}
