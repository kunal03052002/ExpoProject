...
export const getDragPreview = (
  ...
  const canDrop = canCrossDifferenceColumnNumber(
    draggedComponents.map((item:any) => item.layoutInfo),
  )

  const draggedDisplayNames = draggedComponents.map((item:any) => item.displayName)
  const combineWidget = combineWidgetInfos(
    draggedComponents.map((item:any) => item.layoutInfo),
  )
  const currentWidgetLayoutInfo = draggedComponents.find(
    (item:any) => item.displayName === draggedDisplayName,
  )
