...
import { ComponentMapNode, ComponentTreeNode, ComponentsState } from "./componentsState"
// import { getActionList } from "../action/actionSelector"
import { RawTreeFactory } from "@/utils/executionTreeHelper/rawTreeFactory"
import { getAppliedTheme, getClaasesObject } from "../style/styleSelector"
import { RawTreeSeedShape } from "@/utils/executionTreeHelper/interface"
...
const isComponentOrParentDisable = (displayName: string,
  layoutState: Record<string, "disable" | "active">,
  components: ComponentsState) => {
  if (!displayName) {
    return true;
  }
  if (layoutState[displayName] === "active") {
    return false
  }
  if (layoutState[displayName] === "disable") {
    return true
  }
  if (components[displayName].parentNode === "root") {
    if (layoutState.other === "active") {
      return false
    }
    return true
  }
  return isComponentOrParentDisable(components[displayName].parentNode, layoutState, components)
}
export function isCurrentNodeDisable(
  displayName: string = "root",
  rootState: RootState = store.getState(),
) {

  const components = getComponentMap(rootState);
  return isComponentOrParentDisable(displayName, rootState.config.layoutSectionState, components)
}
export function getLayoutSection(rootState: RootState = store.getState()) {
  const components = getComponentMap(rootState)
  const layoutGridName: string = components["bodySection1-bodySectionContainer1"].childrenNode[0];
  const layoutGrid = components[layoutGridName];
  return layoutGrid.childrenNode;
}
...
export const getAllComponentDisplayNameMapProps = createSelector(
  ...
  (componentsMap, pageNameMapDescendantNodeDisplayNames) => {
    ...
    Object.keys(pageNameMapDescendantNodeDisplayNames).forEach((pageName) => {
      ...
      pageNameMapDescendantNodeDisplayNames[pageName].forEach((displayName: any) => {
...
export const getCurrentPageNode = createSelector(
  ...
  (components) => {
    ...
    const currentPage = rootDSL.childrenNode.find(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (nodeDisplayName: any) => nodeDisplayName === currentPageDisplayName,
    )
...
export const getPageDisplayNameMapViewDisplayName = createSelector(
  ...
  (components) => {
    ...
    pageNodes.forEach((pageNode: any) => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      sectionNodes.forEach((sectionNode: any) => {
...
const getActionList = (state: RootState) => state.currentApp.action
export const getRawTree = createSelector(
  ...
  (actions, widgets,
    globalData, classes, theme) => {
    return RawTreeFactory.create({
      actions: [
        ...actions,
        { displayName: "other", resourceId: "123bn", content: "displayName", actionId: "23nb3" }],
      widgets: widgets ?? {},
      globalData,
      classes,
      theme
