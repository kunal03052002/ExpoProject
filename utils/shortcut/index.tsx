...
import { createModal } from "../model/src";
...
import { styleActions } from "@/redux/currentApp/style/cursorSlice";
// import { ILLA_MIXPANEL_EVENT_TYPE } from "./interface"
...
export const Shortcut: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const currentSelectedComponent = useSelector(
    getSelectedComponentDisplayNames
  );
  const currentSelectedAction: any = useSelector(getSelectedAction);
      ...
      const id = modal.show({
        ...
        onOk: () => {
          switch (type) {
            ...
            case "theme": {
              dispatch(styleActions.deleteThemeReducer(displayName[0]));
              dispatch(styleActions.updateCachedTheme(undefined));
              dispatch(configActions.updateSelectedThemePanel("Default_Theme"));
              break;
            }
            case "class": {
              dispatch(styleActions.deleteStyleClassReducer(displayName[0]));
