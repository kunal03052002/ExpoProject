...
import { DEFAULT_ASIDE_COLUMNS_NUMBER, DEFAULT_BODY_COLUMNS_NUMBER } from "../basicConfig"

export const searchForefatherSectionNodeDisplayName = (
  currentDisplayName: string,
): string | null => {
  const rootState = store.getState()
  const widgetsLayoutInfo:any = getClientWidgetLayoutInfo(rootState)
  const currentLayoutInfo:any = widgetsLayoutInfo[currentDisplayName]
...
export const autoChangeWhenClickOnCanvas = (canvasDisplayName: string) => {
  const rootState = store.getState()
  const widgetsLayoutInfo:any = getClientWidgetLayoutInfo(rootState)
  const parentContainerDisplayName:any =
